...
fn run_unsorted_test(run_times: u8) -> (TableEntry, TableEntry) {
    ...
    for _ in 0..run_times {
        let tx_100 = tx_100.clone();
        let tx_1k = tx_1k.clone();
        let tx_10k = tx_10k.clone();
    ...
    for _ in 0..run_times {
        unsorted_time_counts_100_vec.push(rx_100.recv().unwrap());
        unsorted_time_counts_1k_vec.push(rx_1k.recv().unwrap());
...
fn run_sorted_test(run_times: u8) -> (TableEntry, TableEntry) {
    ...
    for _ in 0..run_times {
        sorted_time_counts_100_vec.push(rx_100.recv().unwrap());
        sorted_time_counts_1k_vec.push(rx_1k.recv().unwrap());
